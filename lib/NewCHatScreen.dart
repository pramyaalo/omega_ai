  ...
  class _NewChatScreenState extends State<NewChatScreen> {
    ...
    void _connectWebSocket() {
      ...
      channel!.stream.listen(
            (data) {
          // ── FIX: guard against disposed widget ──────────────
          if (!mounted) return;
          ...
          setState(() {
            ...
            // ── FIX: stream replaces last AI bubble text (cumulative from backend) ──
            if (type == "stream") {
              bool found = false;
              for (int i = messages.length - 1; i >= 0; i--) {
                if (messages[i]["isMe"] == false) {
                  messages[i]["text"]        = text; // backend sends cumulative text
              ...
              // Safety: if no AI bubble exists yet, create one
              if (!found) {
                messages.add({"text": text, "isMe": false});
              }
            }
    ...
    // ── FIX: use _selectedModel everywhere, not hardcoded ──
    void _sendMessage() {
      ...
      if (selectedImage != null) {
        ...
        channel!.sink.add(jsonEncode({
          ...
          "model": _selectedModel, // ← FIX: was hardcoded "Llama 3.1 8B"
        }));
        ...
        builder: (_) => Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            GridView.count(
              ...
              children: [
                _quickActionTile(Icons.image_search_rounded,    "Create Image",      const Color(0xFF1BA8D4), () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const ImageGeneratorScreen())); }),
                _quickActionTile(Icons.psychology_rounded,      "Deep Think",        Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Think deeply and explain: "; }),
                ...
                _quickActionTile(Icons.shopping_bag_rounded,    "Shopping",          Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Best options to buy: "; }),
                _quickActionTile(Icons.science_rounded,         "Research",          Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Research and summarize: "; }),
                _quickActionTile(Icons.school_rounded,          "Study",             const Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Teach me about: "; }),
                _quickActionTile(Icons.explore_rounded,         "Explore",           const Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Explore the topic: "; }),
                _quickActionTile(Icons.calculate_rounded,       "Math",              const Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Solve this: "; }),
                _quickActionTile(Icons.code_rounded,            "Code",              const Color(0xFF1BA8D4), () { Navigator.pop(context); messageController.text = "Write code for: "; }),
              ],
            ),
          ]),
        ),
