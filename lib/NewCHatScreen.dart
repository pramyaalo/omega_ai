...
class _NewChatScreenState extends State<NewChatScreen> {
  ...
  bool _intentionalClose = false; // ← FIX: track intentional closes
  ...
  static const kBgLight     = Color(0xFFF6FAFE);
  static const kSidebarBg   = Color(0xFFFFFFFF);
  ...
  final Map<String, Map<String, String>> _uiText = {
    "English": {
      ...
      "howCanIHelp": "Welcome to", "subTitle": "Your autonomous AI assistant.",
  ...
  // ══════════════════════════════════════════════════════════
  // WEBSOCKET — no reconnect loop fix
  // ══════════════════════════════════════════════════════════
  void _connectWebSocket() {
    ...
    channel!.stream.listen(
        ...
        // Browser open — async, outside setState
        ...
        // File download + share
        if (type == "file_result") {
          final fileB64  = decoded["file"]      as String? ?? "";
          final fileName = decoded["file_name"] as String? ?? "edited_file.txt";
          if (fileB64.isNotEmpty) {
            setState(() {
              for (int i = messages.length - 1; i >= 0; i--) {
                if (messages[i]["isMe"] == false) {
                  messages[i]["fileB64"]  = fileB64;
                  messages[i]["fileName"] = fileName;
                  break;
                }
              }
            });
            try {
              ...
              await Share.shareXFiles([XFile(file.path)], text: 'File edited by Omega AI');
            } catch (e) {
              debugPrint('file_result error: $e');
            }
          }
          return;
        }
        ...
        setState(() {
          ...
          if (type == "stream") {
            ...
            if (!found) messages.add({"text": text, "isMe": false});
          }
      ...
      onError: (error) {
        if (!mounted || _intentionalClose) return;
        // Unexpected error — reconnect after delay
        Future.delayed(const Duration(seconds: 3), () {
          if (mounted && !_intentionalClose) _connectWebSocket();
        });
      },
      onDone: () {
        if (!mounted || _intentionalClose) return;
        // Server closed unexpectedly — reconnect
        Future.delayed(const Duration(seconds: 3), () {
          if (mounted && !_intentionalClose) _connectWebSocket();
        });
      },
  ...
  Future<void> _sendImage(String caption) async {
    ...
    setState(() {
      messages.add({"text": caption.isNotEmpty ? caption : "Image", "isMe": true,
        "imageBase64": base64Image, "imageExt": ext});
      selectedImage = null;
    });
    channel!.sink.add(jsonEncode({
    ...
    for (final session in sessions) {
      ...
      for (final msg in msgs) {
        if ((msg['text'] ?? '').toString().toLowerCase().contains(q))
          results.add({'sessionId': session['id'], 'sessionTitle': session['title'] ?? 'Chat',
            'text': msg['text'] ?? '', 'isMe': msg['isMe']});
      }
    }
    for (final msg in messages) {
      if ((msg['text'] ?? '').toString().toLowerCase().contains(q) && msg['typing'] != true) {
        final alreadyAdded = results.any((r) =>
        r['sessionId'] == currentSessionId && r['text'] == msg['text']);
        if (!alreadyAdded)
          results.add({'sessionId': currentSessionId, 'sessionTitle': 'Current Chat',
            'text': msg['text'] ?? '', 'isMe': msg['isMe']});
      }
    }
  ...
  Widget _highlightText(String text, String query, {bool isMe = false}) {
    ...
    return RichText(text: TextSpan(
        style: TextStyle(fontSize: 13, color: isMe ? Colors.white : kTextPrimary),
        children: spans));
  }

  void _shareChat() {
    if (messages.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_t('noMessagesToShare')))); return;
    }
    final user     = FirebaseAuth.instance.currentUser;
    ...
    Share.share(buffer.toString(), subject: "Omega AI Chat");
  }

  void _showExportSheet() {
    if (messages.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_t('noMessagesToExport')))); return;
    }
        ...
        build: (pw.Context ctx) => [
          pw.Container(
            ...
            child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
              pw.Text('Omega AI', style: pw.TextStyle(color: PdfColors.white, fontSize: 20, fontWeight: pw.FontWeight.bold)),
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          GridView.count(
            ...
            children: [
              _quickActionTile(Icons.image_search_rounded,   "Create Image", kCyan,     () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => const ImageGeneratorScreen())); }),
              _quickActionTile(Icons.psychology_rounded,     "Deep Think",   kCyanDark, () { Navigator.pop(context); messageController.text = "Think deeply and explain: "; }),
              ...
              _quickActionTile(Icons.shopping_bag_rounded,   "Shopping",     kCyan,     () { Navigator.pop(context); messageController.text = "Best options to buy: "; }),
              _quickActionTile(Icons.science_rounded,        "Research",     kCyanDark, () { Navigator.pop(context); messageController.text = "Research and summarize: "; }),
              _quickActionTile(Icons.school_rounded,         "Study",        kCyan,     () { Navigator.pop(context); messageController.text = "Teach me about: "; }),
            ...
            PopupMenuItem(value: 'pin',
                child: Row(children: [
                  Icon(isPinned ? Icons.push_pin_outlined : Icons.push_pin,
                      color: const Color(0xFFFFB300), size: 16),
  ...
  Widget build(BuildContext context) {
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        drawer: Drawer(
          backgroundColor: drawerBg, elevation: 0,
              ...
              child: Row(children: [
                ...
                Expanded(child: _isSearching
                  ...
                  decoration: InputDecoration(
                    hintText: _t('searchHint'), hintStyle: const TextStyle(color: kTextMuted),
              ...
              child: _isSearching && _searchQuery.isNotEmpty
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        final session = sessions.firstWhere(
                                (s) => s['id'] == result['sessionId'], orElse: () => {});
                        if (session.isNotEmpty) _loadSessionFromSearch(session);
                      },
                    ),
                  );
                },
                ...
                child: Column(children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text("${_t('howCanIHelp')} ", style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: textColor)),
                    const Text("Omega AI", style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: kCyan)),
                  ]),
                  ...
                  GridView.count(
                    ...
                    children: _quickCards.map((card) => GestureDetector(
                      onTap: () async {
                        if (card['label'] == 'Edit & return file') {
                          if (selectedFile != null) {
                            messageController.text = "Edit this file and return updated version: ${selectedFile!.name}";
                            return;
                          }
                          final result = await FilePicker.platform.pickFiles(withData: true);
                          if (result != null && result.files.isNotEmpty) {
            ...
            if (!_isSearching) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _isTemporary ? Colors.orange.withOpacity(0.5) : (isDark ? kDarkBorder : kBorderLight),
                    width: 1.2,
                  ),
